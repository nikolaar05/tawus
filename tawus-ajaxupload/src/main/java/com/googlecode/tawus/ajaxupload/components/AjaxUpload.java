...
/**
 * An ajax file uploader using <a href='https://github.com/valums/file-uploader'>Valums File
 * Uploader</a>
 * 
 */
...
public class AjaxUpload extends AbstractField
{
	...
	AjaxUpload(List<UploadedFile> value, FieldValidator<Object> validate,
			MultipartDecoder multipartDecoder, AjaxUploadDecoder ajaxDecoder,
	...
	JSONObject fillArguments()
	{
		...
		spec.put("removeText", messages.get("ajaxupload.remove-text"));
		spec.put("failedText", messages.get("ajaxupload.failed-text"));
		...
		// spec.put("allowedExtensions", allowedExtensions.toString());
		return spec;
	}
	...
	/**
	 * Converts the current list of uploaded files to a JSON object containing a json array with
	 * each element containing the name of the file and a unique key for identification. The unique
	 * key is index of the uploaded file in parameter <code>value</code>
	 * 
	 * @return
	 */

                     ...
                     this._options,
                     {
                        element : null,
                           ...
                           template : '<div class="qq-uploader">'
                                 + '<div class="qq-upload-drop-area"><span>'+ this._options.dropText + '</span></div>'
   ...
   _find : function(parent, type)
   {
      ...
      _onComplete : function(id, fileName, result)
      {
         ...
         if (result.success)
         {
            ...
            item.serverIndex = result.serverIndex;
         }
         else
         {
      ...
      _addToList : function(id, fileName, serverIndex)
      {         
      ...
      {
         var self = this;
         Tapestry.ajaxRequest(this._options.initializeUploadsLink, function(response){
            if(response.responseJSON)
            {
               for ( var i = 0; i < response.responseJSON.uploads.size(); ++i)
               {
                  var file = response.responseJSON.uploads[i];
                  self._addToList(id, file.fileName, file.serverIndex);
               }
            }
         });         
      },
      ...
      _bindCancelEvent : function()
      {
         ...
         qq.attach(list, 'click', function(e)
         {
            ...
            else if (qq.hasClass(target, self._classes.remove))
            {
               qq.preventDefault(e);
               var item = target.parentNode;
               self._handler._options.onRemove(item.serverIndex);
               qq.remove(item);
               self._setSizeField();
            }
         });
...
qq.extend(qq.UploadHandlerXhr.prototype, {
   ...
   add : function(file)
   {
      ...
      /**
       * Sends the file identified by id and additional query params to the
       * server
       * 
       * @param {Object}
